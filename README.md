...
```
前端: Vue 3 + Vite + Pinia + ECharts 5 + vue-echarts
后端: FastAPI (Python) + Kimi (Moonshot AI)
数据库: SQLite (开发) / MySQL/PostgreSQL (生产)
持久化: localStorage (开发) / API + SQLite (生产)
...
详细部署指南请查看 [DEPLOYMENT.md](./DEPLOYMENT.md)，包括：

- ✅ 数据持久化到服务器（不再依赖浏览器 localStorage）
- ✅ Nginx 配置（修复 API 路径问题）
- ✅ MySQL/PostgreSQL 数据源支持
- ✅ 环境变量配置
- ✅ 常见问题排查
...
# 5. 访问 http://your-server:5175/kanban/
```
